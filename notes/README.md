...
- [msix-app-icon-targetsize-unplated.md](msix-app-icon-targetsize-unplated.md) — packaged-app Start-menu icon: copy assets into the package (SDK payload omits them), scale the glyph in icon.svg so it fills the frame, and ship `targetsize`+`altform-unplated` Square44x44 assets to drop the grey tile plate (like Visual Studio).
- [directory-photo-cache-tradeoffs.md](directory-photo-cache-tradeoffs.md) — deliberate tradeoffs in the attendee photo cache: ~30-day stale window, mtime-based prune (not freshness), two separate producer-removes coalescers, two resolve paths per row. Recorded so they aren't re-flagged as bugs.
